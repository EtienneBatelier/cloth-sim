    ...
    static class Sewer
{
    ...
    #nullable enable
    public static (ClothPiece, Hook) HookedRectangle(int a, int b = 0, Vector3D? center = null, Vector3D? normalToPlane = null, List<string>? hookTypes = null)
    {
        if (b == 0) {b = a;}
        ...
        for (int i = 0; i < clothPiece.masses.Count; i++)
        {
            for (int j = 0; j < clothPiece.masses.Count; j++)
            {
                ...
                if (0 < dist && dist < 1.1f) {clothPiece.Connect(i, j, 6f, 1f);}
            }
        }
        ...
        //hookType value: oneSide, twoAdjacentSides, twoOppositeSides, threeSides, allSides, corners 
