...
void VueOpenGL::dessine(Masse   const& a_dessiner)
{
    ...
    point_de_vue.scale(0.25);
    dessineCube(point_de_vue);
}

void VueOpenGL::dessine_ressorts(Tissu const& t, QMatrix4x4 const& point_de_vue)
{
    for (auto const& r : t.get_ressorts())
    {
        prog.setUniformValue("vue_modele", matrice_vue * point_de_vue);
        glBegin(GL_LINES);
        prog.setAttributeValue(CouleurId, 0.5, 0.5, 0.5); // gris
        prog.setAttributeValue(SommetId, r.get_depart().get_position().get_vecteur()[0], r.get_depart().get_position().get_vecteur()[1], r.get_depart().get_position().get_vecteur()[2]);
        prog.setAttributeValue(SommetId, r.get_arrivee().get_position().get_vecteur()[0], r.get_arrivee().get_position().get_vecteur()[1], r.get_arrivee().get_position().get_vecteur()[2]);
    }
}
